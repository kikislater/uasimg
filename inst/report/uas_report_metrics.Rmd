...
```{css echo = FALSE}
/* Additional styles for the flight report html output */
h1.title {
  color: #305230;
  font-size:18px;
}
...
cat("<tr><td>__Area__:</td><td><span id='area_acres'>", round((params$area_m2),2), " m²</span></td></tr>", sep = "")
...
m <- leaflet(width="800px", height="600px",
             options = leafletOptions(maxZoom = 17))
...
if ("gsd" %in% names(params$pts)) {
  hist((params$pts$gsd), col="grey50", breaks=20, main="Ground Sampling Distance", xlab="estimated GSD (cm)", ylab="freq")
}
```

```{r echo=FALSE}
if ("alt_agl" %in% names(params$pts)) {
  hist((params$pts$alt_agl), col="grey50", breaks=20, main=" Altitude", xlab="recorded altitude above launch (m)", ylab="freq")
}
```
...
<p style="color:#556B2F; font-style:italic; font-size:90%; text-align:right;">Created with:<br/><a href="https://ucanr-igis.github.io/uasimg/" target="_blank" rel="noopener">Drone Image Utilities for R. Modified by Sylvain POULAIN for G2OI</a></p>
